...
When `splunkDataSansPro` is unset (or `false`), the `@font-face` block in [`layouts/_partials/chrome/theme-vars.html`](https://github.com/splunk/hugo-theme-splunk-workshop/blob/main/layouts/_partials/chrome/theme-vars.html) is skipped entirely — no Heretto-CDN requests fire. The exampleSite/ demo uses exactly this opt-in.
